...
function isInsideCircle(circle, point) {
  const pointRadius = Math.sqrt((point.x - circle.center.x) ** 2
    + (point.y - circle.center.y) ** 2);
  return pointRadius < circle.radius;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const first = a < b ? a : b;
  const sec = first === a ? b : a;
  const end = isEndIncluded ? ']' : ')';
  return `${start + first}, ${sec + end}`;
}
...
function reverseInteger(num) {
  return parseInt(reverseString(num.toString()), 0);
}
...
function getDigitalRoot(num) {
  const res = num.toString().split('').reduce((acc, el) => acc + Number(el), 0);
